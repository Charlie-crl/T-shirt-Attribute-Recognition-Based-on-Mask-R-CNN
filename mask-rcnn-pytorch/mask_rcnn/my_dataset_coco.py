...
class CocoDetection(data.Dataset):
    ...
    def __init__(self, root, dataset="train", transforms=None, years="2017"):
        ...
        if dataset == "train":
            ...
            with open("my_coco68_indices.json", "w") as f:
                f.write(json_str)
    ...
    def __getitem__(self, index):
        ...
        linux_path_list = path.split("\\")
        linux_path = os.path.join(linux_path_list[0],linux_path_list[1])
        img = Image.open(os.path.join(self.img_root, linux_path)).convert('RGB')
...
if __name__ == '__main__':
    train = CocoDetection("./coco_mydata_big", dataset="train",)
    print(len(train))
    t = train[0]
    print(t[1])
