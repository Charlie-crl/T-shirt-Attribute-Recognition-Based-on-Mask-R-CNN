...
def save_info(coco_evaluator,
    ...
    for i in range(len(classes)):
        stats, _ = summarize(coco_evaluator, catId=i + 1)
...
if __name__ == "__main__":
    ...
    # 数据集的根目录
    parser.add_argument('--data-path', default='coco_mydata_small', help='dataset root')

    # 训练好的权重文件
    parser.add_argument('--weights-path', default='./save_weights/model_29.pth', type=str, help='training weights')

    # batch size(set to 1, don't change)
    parser.add_argument('--batch-size', default=1, type=int, metavar='N',
                        help='batch size when validation.')
    # 类别索引和类别名称对应关系
    parser.add_argument('--label-json-path', type=str, default="my_coco13_indices.json")
